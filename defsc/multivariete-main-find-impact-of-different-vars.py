...
if __name__ == "__main__":
    ...
    for filename in os.listdir(directory):
        ...
        import math
        df['exp_ow_wnd_spd'] = df['ow-wnd-spd'].apply(lambda x: math.exp(x))

        columns = ['airly-pm1', 'exp_ow_wnd_spd']

        i = 0

        for column_id in range(len(columns)):
            x_column_names = [predicted_column]
            y_column_names = [predicted_column]

            if columns[column_id] != predicted_column:
                x_column_names.append(columns[column_id])
